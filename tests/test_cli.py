...
class CLITestCase(BaseTestCase):
    ...
    def test_init_command(self):
        result = self.runner.invoke(args=['init', '--username', 'jack', '--password', '12345678'])
        ...
        self.assertEqual(Admin.query.first().username, 'jack')
        self.assertEqual(Category.query.first().name, 'Default')

    def test_init_command_with_update(self):
        self.runner.invoke(args=['init', '--username', 'jack', '--password', '12345678'])
        result = self.runner.invoke(args=['init', '--username', 'new jack', '--password', 'abcdefgh'])
        ...
        self.assertEqual(Admin.query.count(), 1)
        self.assertEqual(Admin.query.first().username, 'new jack')
        self.assertEqual(Category.query.first().name, 'Default')
